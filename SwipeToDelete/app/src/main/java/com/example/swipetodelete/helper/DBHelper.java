...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        // 建立應用程式需要的表格
        db.execSQL(DAOShop.CREATE_TABLE);
        db.execSQL(DAODrink.CREATE_TABLE());
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // 刪除原有的表格
        db.execSQL("DROP TABLE IF EXISTS " + DAOShop.TABLE_NAME);
        db.execSQL("DROP TABLE IF EXISTS " + DAODrink.TABLE_NAME);
        // 呼叫onCreate建立新版的表格
        onCreate(db);
    }
}